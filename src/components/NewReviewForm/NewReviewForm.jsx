...
const reducer = (state, action) => {
    switch (action.type) {
        case actionTypes.changeName: return { name: action.payload, text: '', rating: 0};
        case actionTypes.changeText: return {...state, text: action.payload };
        case actionTypes.changeRating: return {...state, rating: action.payload };
...
export const NewReviewForm = () => {
    ...
    return (
        <div className={styles.root}>
            ...
            <Rating value={formState.rating} size="small" editable onChange={rating => 
                dispatch({type: actionTypes.changeRating, payload: rating})
            }/>
        </div>
    );
}